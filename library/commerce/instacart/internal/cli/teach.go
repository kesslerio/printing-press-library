...
// instacart-pp-cli CLI: `teach` (fire-and-forget, silent on success,
...
import (
	...
	"github.com/mvanhorn/printing-press-library/library/commerce/instacart/internal/learn"
	"github.com/mvanhorn/printing-press-library/library/commerce/instacart/internal/learn/entities"
	"github.com/mvanhorn/printing-press-library/library/commerce/instacart/internal/learn/lookups"
	"github.com/mvanhorn/printing-press-library/library/commerce/instacart/internal/learn/patterns"
...
const noLearnEnvVar = "INSTACART_NO_LEARN"
...
func learningsStateDir() (string, error) {
	...
	dir := filepath.Join(home, ".local", "share", "instacart-pp-cli")
...
func learnDBPath(explicit string) string {
	...
	return defaultDBPath("instacart-pp-cli")
}
...
~/.local/share/instacart-pp-cli/teach.log, safe to fire-and-forget.

Disabling: pass --no-learn or set ` + noLearnEnvVar + `=true.`,
		Example: `  instacart-pp-cli teach --query "<question>" --resource-type <type> \
...
when learnings exist.`,
		Example: `  instacart-pp-cli recall "<question>" --agent
  instacart-pp-cli recall "<question>" --agent --min-confidence 2`,
...
func newLearningsListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  instacart-pp-cli learnings list --agent
  instacart-pp-cli learnings list --query "<substring>"
  instacart-pp-cli learnings list --warnings --agent`,
		Annotations: map[string]string{"mcp:read-only": "true"},
		RunE: func(cmd *cobra.Command, args []string) error {
			if learnDryRunOK(flags) {
				return nil
...
Requires at least one of --resource, --action, or --all.`,
		Example: `  instacart-pp-cli learnings forget "<question>" --resource <id>
  instacart-pp-cli learnings forget "<question>" --all`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if len(args) == 0 {
				return cmd.Help()
			}
...
into a whole family.`,
		Example: `  instacart-pp-cli teach-pattern \
...
cannot be taught — they are derived from the canonical input.`,
		Example: `  instacart-pp-cli teach-lookup --kind country --canonical "United States" --alias USA`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if learnDryRunOK(flags) {
				return nil
			}

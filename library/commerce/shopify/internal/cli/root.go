// Copyright 2026 benjamin-huang. Licensed under Apache-2.0. See LICENSE.
...
func newRootCmd(flags *rootFlags) *cobra.Command {
	rootCmd := &cobra.Command{
		Use:   "shopify-pp-cli",
		Short: `Operate a Shopify store from the terminal with local sync, analytics, and bulk exports.`,
		Long: `Operate a Shopify store from the terminal with local sync, analytics, and bulk exports.

Add --agent to any command for JSON output + non-interactive mode.
Run 'shopify-pp-cli doctor' to verify auth and connectivity.`,
		SilenceUsage: true,
		Version:      version,
	}
	rootCmd.SetVersionTemplate("shopify-pp-cli {{ .Version }}\n")
	...
	rootCmd.PersistentFlags().StringVar(&flags.profileName, "profile", "", "Apply values from a saved profile (see 'shopify-pp-cli profile list')")
	...
	customersCmd.AddCommand(newCustomersTagCmd(flags))
	rootCmd.AddCommand(customersCmd)
	rootCmd.AddCommand(newFulfillmentOrdersCmd(flags))
	rootCmd.AddCommand(newInventoryItemsCmd(flags))
	ordersCmd := newOrdersCmd(flags)
	ordersCmd.AddCommand(newOrdersTagCmd(flags))

...
This CLI ships a self-learning loop. When you run instacart-pp-cli commands, the binary
...
`instacart-pp-cli --no-learn <command>`) to disable the loop's side effects for
one invocation.
